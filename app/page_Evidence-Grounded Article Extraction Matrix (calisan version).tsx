...
function buildRecord(fileName: string, fullText: string, sections: SectionChunk[]) {
  ...
  for (const item of evidenceMap) {
     record[item.field] = item.shortAnswer;
  }
...
export default function Page() {
  ...
  function downloadEvidenceCsv() {
    const header = "Field,Evidence Sentence,Confidence,LLM Validation\n";   
   
    const body = evidenceMap
      .map((e) =>
        [e.field, e.value, e.evidence, e.confidence, e.llmValidation || ""]
